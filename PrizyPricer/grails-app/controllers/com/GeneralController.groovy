...
class GeneralController {
   ...
   def researcher (){
      ...
      if(product_ID){
         ...
         def result  = sql.rows("select (AVG(prices_PRICE)*1.2) as IDEALPRICE from Prices " +
                                "where prices_BAR_CODE = :bc and prices_PRICE not in " +
                                "(select prices_PRICE from Prices where prices_BAR_CODE = :bc " +
                                "order by prices_PRICE desc limit 2 ) and prices_PRICE not in " +
                                "(select prices_PRICE from Prices where prices_BAR_CODE = :bc " +
                                "order by prices_PRICE asc limit 2)",[bc:Long.parseLong(product_BAR_CODE)])

         def idealPrice = result[0][0]
         ...
         product.executeUpdate("update Product set product_HIGHEST_PRICE = ?, product_AVERAGE_PRICE = ?, product_LOWEST_PRICE = ?," +
                                                    "product_NUMBER_OF_PRICES = ?, product_IDEAL_PRICE = ? where product_ID = ?", [max, avg , min , total, idealPrice, Long.parseLong(product_ID)])
      }
